...
> 🗺️ **Estás en:** 🔌 **UD 5 · Sockets TCP y UDP** → 07 · Cuándo usar cada protocolo
...
Este punto junta todo lo anterior (la tabla del [punto 1](/ApuntesPSP/04-sockets-tcp-y-udp/01-que-es-un-socket) y los males de los datagramas del [punto 5](/ApuntesPSP/04-sockets-tcp-y-udp/05-cliente-y-servidor-udp)) para darte la regla de decisión que usarás de por vida como programador.
...
| **Streaming en vivo no-HTTP** (RTP) | **UDP** | Prefieres el directo fluido a un reenvío que lo congele |
| **Juegos online** (Fortnite, Minecraft) | **UDP** | Cientos de paquetes por segundo; la latencia manda |
| NTP (hora) | **UDP** | Manda muchas peticiones y promedia (punto 6) |

Fíjate en el patrón: **cuando el dato es valioso y debe quedar intacto, TCP; cuando lo valioso es el momento, UDP**. Ojo: el *streaming web* moderno (Twitch, YouTube) no usa UDP, sino HTTP sobre TCP (segmentación HLS/DASH) porque busca llegar a todo el mundo por infraestructura web.

---

## 🔀 Y si quieres ambas cosas: QUIC

¿Fiabilidad de TCP con velocidad de UDP? Eso existe: **QUIC**, el protocolo sobre el que funciona **HTTP/3**. QUIC se construye sobre UDP (la capa de transporte es UDP) pero añade por su cuenta confirmaciones, control de congestión y cifrado. Es la "vuelta a la tortilla": la industria decide cuándo incluso el "no fiable" merece una capa de fiabilidad.
...
2. Porque la consulta DNS es **diminuta** y repetirla cuesta casi nada: si la respuesta se pierde, se manda otra petición. La fiabilidad se consigue re-preguntando, no con un canal caro.
...
📚 [Volver al índice de la unidad](/ApuntesPSP/04-sockets-tcp-y-udp) · **Anterior:** [06 · HTTP y NTP](/ApuntesPSP/04-sockets-tcp-y-udp/06-http-y-ntp) · **Siguiente:** [08 · Práctica eco](/ApuntesPSP/04-sockets-tcp-y-udp/08-practica-eco)
